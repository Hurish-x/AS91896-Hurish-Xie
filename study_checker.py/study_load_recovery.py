    ...
    while True:
        ...
        try:
            ...
            if number > max or number < min:
                msgbox(f"Your enter should be between {min} and {max}")
                continue
...
def get_valid_int(question, min, max):
    ...
    while True:
        ...
        try:
            ...
            msgbox("Please enter an integer number,such as 3,4,5")
            continue
        return number
...
def add_daily_log(records):
    ...
    if time in records:
        overwrite_choice = buttonbox(
            f"A log already exists for {time}.\n"
    ...
    social_time = get_valid_float(
        "How much time did you connect with friends or family?", 0, 24)
    ...
    msgbox("The log has been saved successfully", app_title)
    return records
...
def edit_log(records):
    ...
    if len(date_choices) == 1:
        date_to_edit = date_choices[0]
    else:
    ...
    if field_to_edit in ["Sleep Time", "Stress Level", "Focus Level"]:
        if field_to_edit == "Sleep Time":
            new_value = get_valid_float(f"Enter the new {field_to_edit}:", 0,
                                        24)
        if field_to_edit in ["Stress Level", "Focus Level"]:
            new_value = get_valid_int(f"Enter the new {field_to_edit}:", 1,
                                      5)
        if new_value is None:
            return records
...
def delete_log(records):
    ...
    if len(date_choices) == 1:
        date_to_delete = date_choices[0]
    else:
        date_to_delete = choicebox("Choose a date to delete:", app_title,
                                   date_choices)
...
def analyse_records(records):
    ...
    analysis_text += f"Average focus level: {average_focus:.1f} / 5\n"
    ...
    analysis_text += f"Average goal completion: {average_goal:.1f} / 5\n\n"
...
def generate_graph(records):
    ...
    ax2 = ax1.twinx()
    ax2.plot(dates, focus_levels, c="red", marker="s", linestyle="--",
             label="Focus Level")
    ...
    fig.tight_layout()
    plt.grid(True)
    file_path = os.path.join(analysis_image_folder, "sleep_focus_graph.png")
    ...
    ax2.plot(dates, goal_completion, color="cyan", marker="o", linewidth=2,
             label="Goal Completion")

    ax2.plot(dates, stress_levels, color="brown", marker="s", linestyle="--",
             linewidth=2, label="Stress Level")

    ax2.set_xlabel("Date")
    ax2.set_ylabel("Level (1-5)")

    # Both variables use the same 1-5 scale.
    ax2.set_ylim(0.5, 5.5)
    ax2.set_yticks([1, 2, 3, 4, 5])

    ax2.set_title("Goal Completion and Stress Level")
    ...
    file_path = os.path.join(analysis_image_folder, "goal_stress_graph.png")
    ...
    plt.tight_layout()
    file_path = os.path.join(analysis_image_folder,
                             "average_time_bar_graph.png")
    plt.savefig(file_path)
    plt.close()
